...
# Cache for server custom prefixes
prefix_cache = {}

# ==========================================
# DYNAMIC PREFIX FUNCTION (PER-SERVER, SERVER OWNER & BOT OWNER PREFIXLESS)
# ==========================================
def get_prefix(bot, message):
    ...
    - Default prefix: ','
    - Bot Owners (IDs: 1159048734559195156, 141314458268303393) can use prefixless commands everywhere.
    - Server Owners (Ownership ONLY) can use prefixless commands in their own server!
    """
...
# ==========================================
# FUN COMMAND GIF COLLECTIONS (DIRECT GIF URLS)
# ==========================================
KISS_GIFS = [
...
bot = commands.Bot(
    command_prefix=get_prefix,  # Dynamic prefix per server
    intents=intents, 
    help_command=None,
    case_insensitive=True,
    owner_ids=OWNER_IDS  # Configures multiple bot owners
)
tree = bot.tree
...
# ==========================================
# 🆕 TICKET BUTTON UI COMPONENTS (WITH STAFF PING)
# ==========================================
...
class TicketLaunchView(discord.ui.View):
    ...
    async def open_ticket_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Grant access & gather Staff role mentions
        ...
        # Build notification ping string for Staff & Ticket opener
        ...
        # Pings staff roles upon channel creation
...
class HelpDropdown(discord.ui.Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(label="Server Setup", description="SetPrefix, Setup, SetModLog, Welcome...", emoji="⚙️"),
            ...
            discord.SelectOption(label="Economy & Rewards", description="Daily, Balance, Pay...", emoji="💰"),
            ...
            discord.SelectOption(label="Bot Owner", description="AddPremium, Blacklist, Tuff, Leave...", emoji="👑"),
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if category == "Moderation":
            ...
            embed.description = (
                f"`{curr_pref}setprefix <new_prefix>` - Change server command prefix\n"
                f"`{curr_pref}prefix` - View current server prefix\n"
                f"`{curr_pref}setup` - Auto-create moderation roles & channels\n"
                f"`{curr_pref}setmodlog #channel` - Set mod-log channel for this server\n"
                f"`{curr_pref}setwelcome #channel` - Set welcome channel\n"
                f"`{curr_pref}setleave #channel` - Set leave channel\n"
                f"`{curr_pref}say <message>` - Make the bot send a message\n"
                f"`{curr_pref}announce <message>` - Send an announcement embed"
            )
            ...
            embed.description = (
                ...
                f"`{curr_pref}balance` | `{curr_pref}bal [@user]` - View coin balance\n"
...
async def on_ready():
    ...
    # Pre-load custom prefixes into memory cache
...
async def on_message(message):
    ...
    # Check if message author is returning from AFK
...
async def on_command_error(ctx, error):
    # Ignore CommandNotFound silently for prefixless owner commands & normal messages
...
async def remind(ctx, time_str: str, *, reminder: str):
    ...
    if seconds <= 0 or seconds > 86400 * 30:  # Max 30 days
...
async def mimic(ctx):
    ...
    # Prepare edit payload strictly for guild.me (SERVER-SPECIFIC PROFILE)
    ...
    try:
        ...
        # Fallback if discord.py's Member.edit doesn't support 'banner' parameter
        ...
        try:
            ...
            return await status_msg.edit(content=None, embed=discord.Embed(
                ...
                description="The bot lacks permissions to edit its server profile (`Change Nickname` or server avatar permissions).",
                color=discord.Color.red()
            ))
...
async def blacklist(ctx, user: discord.User):
    ...
    await ctx.send(f"🚫 {user} has been blacklisted from adding this bot.")
